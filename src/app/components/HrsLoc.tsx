        ...
        <div className="flex flex-col items-center w-full">
          {locations.map((location, index) => (
            <button
              ...
              className={`w-auto mb-3 border rounded-lg p-2 ${
                selectedLocation === index ? "bg-black bg-opacity-10 font-bold" : ""
              }`}
            >
        ...
        {locations.map((location, index) => (
          <div
            ...
            className={`flex transition-all duration-1000 ease-in-out ${
              selectedLocation === index ? "max-h-screen opacity-100" : "max-h-0 opacity-0"
            }`}
          >
      ...
      {locations.map((location, index) => (
        <div
          ...
          className={`flex justify-center transition-all duration-1000 ease-in-out ${
            selectedLocation === index ? "h-1/2 opacity-100" : "max-h-0 opacity-0"
          }`}
        >
          <img
            src={`${location.image || 'rstock.jpg'}`}
            alt="Location Image"
            className="w-full object-cover h-40 sm:h-96"
          />
        </div>
      ))}
